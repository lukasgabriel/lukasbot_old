...
err_response = (
    "BLEEP BLOOP I'M MALFUNCTIONING PLEASE CALL MY CREATOR @flyomotive TO FIX ME!"
)
...
@start.bot.command(
    name="hello",
    help="You should show some manners and greet the bot. He is our humble servant after all.",
)
async def hello(ctx):
...
@start.bot.command(
    name="cat",
    help="Shows you a picture of a cat that doesn't exist. A fictious feline.",
)
async def cat(ctx):
    cat = get_cat()
...
@start.bot.command(
    name="person", help="Shows you a picture of a person that doesn't exist."
)
...
@start.bot.command(
    name="whattoplay", help="I'll give you a suggestion what you could play together"
)
...
@start.bot.command(
    name="addgame",
    help="Add a game to the list of games that I suggest when you call "
    ">whattoplay"
    "",
)
async def addgame(ctx):
    ...
    record = (
        author + " suggested the game " + game_wish + " via the '>addgame' command."
    )
...
@start.bot.command(
    name="addresses",
    help="Returns all address book entries that currently have their phone number set and can receive SMS messages.",
)
...
@start.bot.command(
    name="sms",
    help="Send an SMS. Format your request like so:  MESSAGE : RECIPIENT  -  you can view the command's address book with the '>addresses' command.",
)
...
async def sms(ctx):
    ...
    try:
        ...
        msg = f"Message from {author}: " + msg_raw.rsplit(":", 1)[0].strip()
...
@start.bot.command(
    name="twitch_notify",
    help="Currently WIP. Format your command like so: STREAMER_NAME : [on/off]",
)
...
async def twitch_notify(ctx):
    ...
    try:
        ...
        msgresponse = (
            ">>> Invalid command format. Use '>help twitch_notify' for more info."
        )
    except ():
...
@start.bot.command(
    name="dice",
    help="Roll a dice. You can choose how many sides the dice should have (default is 6).",
)
async def dice(ctx):
...
@start.bot.command(
    name="urban",
    help="You can look up the urban dictionary definition for a word using this command.",
)
